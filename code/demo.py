...
for i, data_i in enumerate(dataloader):
    ...
    if opt.save_per_img:
        ...
        try:
            ...
            # for j in range(out[0].data.cpu().shape[0]):
            #     # vutils.save_image((out.data.cpu()[j]+1)/2, save_root + '/test/' + opt.name + '/split/' + str(i)+'_'+str(j) + '.png',  
            #     #     nrow=imgs_num, padding=0, normalize=False)
            #     # print(imgs.shape)
                
            #     vutils.save_image(imgs[:,j], save_root + '/test/' + opt.name + '/split/' + str(i)+'_'+str(j) + '.png',  
            #         nrow=1,  padding=0, normalize=False)
        except OSError as err:
            print(err)
