...
class Pix2PixTrainer():
    ...
    def __init__(self, opt, resume_epoch=0):
        ...
        if opt.use_ema:
            ...
            self.netBkg_ema = EMA(opt.ema_beta)
            for name, param in self.pix2pix_model_on_one_gpu.net['background'].named_parameters():
                if param.requires_grad:
                    self.netBkg_ema.register(name, param.data)

        self.generated = None
        if opt.isTrain:
            # self.optimizer_G, self.optimizer_D = \
            
            self.optimizer_G, self.optimizer_D, self.optimizer_B=\
            ...
            if opt.continue_train and opt.which_epoch == 'latest':
                ...
                self.optimizer_B.load_state_dict(checkpoint['B'])
        self.last_data, self.last_netCorr, self.last_netG, self.last_netBkg, self.last_optimizer_G = None, None, None, None, None  
    ...
    def run_generator_one_step(self, data, alpha=1):
        ...
        if self.opt.use_ema:
            ...
            self.netBkg_ema(self.pix2pix_model_on_one_gpu.net['background'])
    ...
    def run_unet(self,data):
        ...
        GforD['fake_image'] = self.out['fake_image']
        # GforD['result'] = self.out['result']
        b_losses = self.pix2pix_model(data,mode='bg',GforD = GforD)
        b_loss = sum(b_losses.values()).mean()
        b_loss.backward()
        self.optimizer_B.step()
        self.b_losses = b_losses
    ...
    def save(self, epoch):
        ...
        if self.opt.use_ema:
            ...
            self.netBkg_ema.assign(self.pix2pix_model_on_one_gpu.net['background'])
            util.save_network(self.pix2pix_model_on_one_gpu.net['background'], 'netBkg_ema', epoch, self.opt)
            self.netBkg_ema.resume(self.pix2pix_model_on_one_gpu.net['background'])
            ##############修改###########
        if epoch == 'latest':
    ...
    def update_fixed_params(self):
        ...
        B_params =  [{'params': self.pix2pix_model_on_one_gpu.net['background'].parameters(), 'lr': self.opt.lr*0.5}]
        ...
        self.optimizer_B = torch.optim.Adam(B_params, lr=B_lr, betas=(beta1, beta2), eps=1e-3)
