...
import models.networks.background.networks as bkgnetworks
...
class Pix2PixModel(torch.nn.Module):
    ...
    def forward(self, data, mode, GforD=None, alpha=1):
        ...
        if mode == 'generator':
            ...
            g_loss, generated_out = self.compute_generator_loss(input_label,
                                                                input_semantics, real_image, ref_label, ref_semantics, ref_image, self_ref, bg, bg_gray, bg_edge,bg_mask)
            ...
            with torch.no_grad():
                ...
                fake_bg = self.generate_fake_bg(
                    image=bg, bg_gray=bg_gray, bg_edge=bg_edge, bg_mask=bg_mask)
                out = self.inference(input_semantics, ref_semantics=ref_semantics, ref_image=ref_image,
                                     self_ref=self_ref, real_image=ref_image,  bg=fake_bg, input_label=input_label)
    ...
    def create_optimizers(self, opt):
        G_params, D_params, B_params = list(), list(), list()
        G_params += [{'params': self.net['netG'].parameters(),
                      'lr': opt.lr*0.5}]
        G_params += [{'params': self.net['netCorr'].parameters(),
                      'lr': opt.lr*0.5}]
        G_params += [{'params': self.net['background_edge'].parameters(),
                      'lr': opt.lr*0.005}]
        G_params += [{'params': self.net['background_painting'].parameters(),
                      'lr': opt.lr*0.005}]
        ...
        if opt.no_TTUR:
            ...
            # G_lr, D_lr, B_lr = opt.lr, opt.lr, opt.lr
        else:
            beta1, beta2 = 0, 0.9
            G_lr, D_lr = opt.lr / 2, opt.lr * 2
            # G_lr, D_lr, B_lr = opt.lr / 2, opt.lr * 2, opt.lr / 2

        optimizer_G = torch.optim.Adam(
            G_params, lr=G_lr, betas=(beta1, beta2), eps=1e-3)
    ...
    def save(self, epoch):
        ...
        util.save_network(self.net['background_edge'],
                          'Bkg_Edge', epoch, self.opt)
        util.save_network(self.net['background_painting'],
                          'Bkg_painting', epoch, self.opt)

        if self.opt.weight_domainC > 0 and self.opt.domain_rela:
            util.save_network(
                self.net['netDomainClassifier'], 'DomainClassifier', epoch, self.opt)

    
    def initialize_networks(self, opt):
        # print('net load begin')
        net = {}
        ...
        net['background_edge'] = bkgnetworks.EdgeGenerator(
            use_spectral_norm=True)
        net['background_painting'] = bkgnetworks.InpaintGenerator()
        net['background_edge'] = util.load_network(
                net['background_edge'], 'Bkg_Edge', opt.which_epoch, opt)
        net['background_painting'] = util.load_network(
                net['background_painting'], 'Bkg_painting', opt.which_epoch, opt)
        print('Bkg networks has load weights')
        
        
        if not opt.isTrain or opt.continue_train:
            net['netG'] = util.load_network(
                net['netG'], 'G', opt.which_epoch, opt)
            if opt.isTrain:
                net['netD'] = util.load_network(
                    net['netD'], 'D', opt.which_epoch, opt)
            net['netCorr'] = util.load_network(
                net['netCorr'], 'Corr', opt.which_epoch, opt)
            
            if opt.weight_domainC > 0 and opt.domain_rela:
                net['netDomainClassifier'] = util.load_network(
                    net['netDomainClassifier'], 'DomainClassifier', opt.which_epoch, opt)
            if (not opt.isTrain) and opt.use_ema:
                net['netG'] = util.load_network(
                    net['netG'], 'G_ema', opt.which_epoch, opt)
                net['netCorr'] = util.load_network(
                    net['netCorr'], 'netCorr_ema', opt.which_epoch, opt)
                
                net['background_edge'] = util.load_network(
                    net['background_edge'], 'Bkg_Edge_ema', opt.which_epoch, opt)
                net['background_painting'] = util.load_network(
                    net['background_painting'], 'Bkg_Painting_ema', opt.which_epoch, opt)
                
        # print('net load ending')
        if len(opt.gpu_ids) > 0:
            net['background_edge'].cuda()
            net['background_painting'].cuda()
        # print('corr.device',next(net['netCorr'].parameters()).device)
        # print('background_edge.device',next(net['background_edge'].parameters()).device)
        return net
    ...
    def preprocess_input(self, data):
        ...
        if self.use_gpu():
            ...
            data['bg_gray'] = data['bg_gray'].cuda()
            data['bg_edge'] = data['bg_edge'].cuda()
            data['bg_mask'] = data['bg_mask'].cuda()
        ...
        return data['label'], input_semantics, data['image'], data['self_ref'], data['ref'], data['label_ref'], ref_semantics, data['bg'],data['bg_gray'],data['bg_edge'],data['bg_mask']


    def get_ctx_loss(self, source, target):
        contextual_style5_1 = torch.mean(
            self.contextual_forward_loss(source[-1], target[-1].detach())) * 8
        contextual_style4_1 = torch.mean(
            self.contextual_forward_loss(source[-2], target[-2].detach())) * 4
        contextual_style3_1 = torch.mean(self.contextual_forward_loss(
            F.avg_pool2d(source[-3], 2), F.avg_pool2d(target[-3].detach(), 2))) * 2
        if self.opt.use_22ctx:
            contextual_style2_1 = torch.mean(self.contextual_forward_loss(
                F.avg_pool2d(source[-4], 4), F.avg_pool2d(target[-4].detach(), 4))) * 1
    ...
    def compute_generator_loss(self, input_label, input_semantics, real_image, ref_label=None, ref_semantics=None, ref_image=None, self_ref=None, bg=None, bg_gray=None, bg_edge=None, bg_mask=None):
        ...
        generate_out = self.generate_fake(
            input_semantics, real_image, ref_semantics=ref_semantics, ref_image=ref_image, self_ref=self_ref, bg=fake_bg, input_label=input_label)
        ...
        if self.opt.warp_cycle_w > 0:
            ...
            G_losses['G_warp_cycle'] = F.l1_loss(
                generate_out['warp_cycle'], ref) * self.opt.warp_cycle_w
        ...
        if self.opt.warp_self_w > 0:
            ...
            sample_weights = (self_ref[:, 0, 0, 0] / (
                sum(self_ref[:, 0, 0, 0]) + 1e-5)).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            G_losses['G_warp_self'] = torch.mean(F.l1_loss(
                generate_out['warp_out'], real_image, reduce=False) * sample_weights) * self.opt.warp_self_w
        ...
        sample_weights = (self_ref[:, 0, 0, 0] / (sum(self_ref[:, 0, 0, 0]
        ...
        for i in range(len(generate_out['real_features'])):
            loss += weights[i] * util.weighted_l1_loss(
                fake_features[i], generate_out['real_features'][i].detach(), sample_weights)
        ...
        if self.opt.warp_mask_losstype != 'none':
            ref_label = F.interpolate(
                ref_label.float(), scale_factor=0.25, mode='nearest').long().squeeze(1)
            gt_label = F.interpolate(
                input_label.float(), scale_factor=0.25, mode='nearest').long().squeeze(1)
            weights = []
            ...
            G_losses['mask'] = (F.nll_loss(torch.log(generate_out['warp_mask'] + 1e-10), gt_label,
                                reduce=False) * weights).sum() / (weights.sum() + 1e-5) * self.opt.weight_mask
    ...
    def generate_fake_bg(self, image=None, bg_gray=None, bg_edge=None, bg_mask=None):
        # self.net['background_edge'].eval()
        # self.net['background_painting'].eval()
        # print('torch.unique(bg_mask)',torch.unique(bg_mask))
        bg_gray_masked = (bg_gray * (1 - bg_mask)) + bg_mask
        edges_masked = (bg_edge * (1 - bg_mask))
        inputs = torch.cat((bg_gray_masked, edges_masked, bg_mask), dim=1)

        reconstruct_edge = self.net['background_edge'](inputs)
        image_masked = (image * (1 - bg_mask).float()) + bg_mask
        
        inputs = torch.cat((image_masked, reconstruct_edge), dim=1)
        reconstruct_painting = self.net['background_painting'](inputs)

        reconstruct_bg = (reconstruct_painting * bg_mask) + (image * (1 - bg_mask))
    
        return reconstruct_bg*2-1

    
    def generate_fake(self, input_semantics, real_image, ref_semantics=None, ref_image=None, self_ref=None, bg=None, input_label=None):
        generate_out = {}

        ref_relu1_1, ref_relu2_1, ref_relu3_1, ref_relu4_1, ref_relu5_1 = self.vggnet_fix(
            ref_image, ['r12', 'r22', 'r32', 'r42', 'r52'], preprocess=True)

        coor_out = self.net['netCorr'](
            ref_image, real_image, input_semantics, ref_semantics, alpha=self.alpha)

        
        generate_out['ref_features'] = [ref_relu1_1,
                                        ref_relu2_1, ref_relu3_1, ref_relu4_1, ref_relu5_1]
        generate_out['real_features'] = self.vggnet_fix(
            real_image, ['r12', 'r22', 'r32', 'r42', 'r52'], preprocess=True)
        ...
        mask = input_label != 0
        # generate_out['fake_image']=generate_out['fake_image_fg']*mask+generate_out['construct_bg']*(~mask)
        generate_out['fake_image'] = generate_out['fake_image_fg'] * \
            (mask.detach())+generate_out['construct_bg']*((~mask).detach())

        generate_out = {**generate_out, **coor_out}
        return generate_out

    
    def inference(self, input_semantics, ref_semantics, ref_image, self_ref, real_image, bg, input_label):
        generate_out = {}
        coor_out = self.net['netCorr'](
            ref_image, None, input_semantics, ref_semantics, alpha=self.alpha)
        if self.opt.CBN_intype == 'mask':
            CBN_in = input_semantics
        elif self.opt.CBN_intype == 'warp':
            CBN_in = coor_out['warp_out']
        ...
        return generate_out['fake_image'],generate_out['construct_bg']
    ...
    def discriminate(self, input_semantics, fake_image, real_image):
        ...
        if self.opt.D_cam > 0:
            fake_cam_logit = torch.cat(
                [it[:it.shape[0]//2] for it in cam_logit], dim=1)
            real_cam_logit = torch.cat(
                [it[it.shape[0]//2:] for it in cam_logit], dim=1)
    ...
    def compute_D_seg_loss(self, out, gt):
        ...
        down_gt = F.interpolate(gt.unsqueeze(1).float(
        ), scale_factor=0.5, mode='nearest').squeeze().long()
        fake_seg_loss_down = F.cross_entropy(fake_seg[0][1], down_gt)
        real_seg_loss_down = F.cross_entropy(real_seg[0][1], down_gt)

        seg_loss = fake_seg_loss + real_seg_loss + \
            fake_seg_loss_down + real_seg_loss_down
        return seg_loss
