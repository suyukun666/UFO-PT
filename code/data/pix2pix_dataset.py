...
class Pix2pixDataset(BaseDataset):
    ...
    def get_label_tensor(self, path):
        ...
        label_tensor[label_tensor==1] = self.opt.label_nc  # 'unknown' is opt.label_nc
        return label_tensor, params1,label

    def __getitem__(self, index):

        label_path = self.label_paths[index]
        
        label_tensor, params1,label_img = self.get_label_tensor(label_path)
        ...
        if random_p < self.real_reference_probability or self.opt.phase == 'test':
            ...
            # print('1.Image.open',np.array(image_ref).shape)(450, 500, 3)
            ...
            label_ref_tensor, params,label_ref = self.get_label_tensor(path_ref_label)
            ...
            if self.opt.dataset_mode == 'deepfashion' and self.opt.video_like:
                ...
                if os.path.dirname(ref_name) == os.path.dirname(key_name) and os.path.basename(ref_name).split('_')[0] == os.path.basename(key_name).split('_')[0]:
                    ...
                    label_ref_tensor, params,label_ref= self.get_label_tensor(label_ref_path)
                    transform_image = get_transform(self.opt, params)
                    ref_tensor = transform_image(image_ref) 

                    pair = True
            if not pair:
                label_ref_tensor, params,label_ref = self.get_label_tensor(label_path)
                transform_image = get_transform(self.opt, params)
        ...
        # bg= np.array(Image.open(path_ref).convert('RGB').resize((256,256),Image.BILINEAR))
        if image_ref!=None:
            bg= np.array(image_ref.resize((256,256),Image.BILINEAR))
            bg_label=np.array(label_ref.resize((256,256),Image.BILINEAR))
        else:
            bg=np.array(image.resize((256,256),Image.BILINEAR))
            bg_label=np.array(label_img.resize((256,256),Image.BILINEAR))

        bg_tensor=self.to_tensor(bg)
        # bg_label_pth=path_ref.replace('.jpg','.png')
        # bg_label=np.array(Image.open(bg_label_pth).convert('L').resize((256,256),Image.BILINEAR))
        
        bg_label = rgb2gray(bg_label)
        bg_label = (bg_label > 0).astype(np.uint8) * 255

        img=bg
        bg_gray=rgb2gray(img)
        bg_mask_tensor=self.to_tensor(bg_label)
       
        edge_mask=None if self.opt.phase=='train' else (1 -bg_label/255).astype(np.bool)
        
        bg_edge=canny(bg_gray, sigma=2, mask=edge_mask).astype(np.float)
        
        bg_gray_tensor=self.to_tensor(bg_gray)
        bg_edge_tensor=self.to_tensor(bg_edge)
        # vutils.save_image(bg_edge_tensor,'visual/read_data/'+path_ref.split('/')[-1])
        ...
        input_dict = {'label': label_tensor,
                      ...
                      'bg': bg_tensor,
                      'bg_gray':bg_gray_tensor,
                      'bg_edge':bg_edge_tensor,
